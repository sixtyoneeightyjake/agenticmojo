.PHONY: help uv-venv uv-sync install-dev uv-pre-commit uv-test test pre-commit fix-format pre-commit-install pre-commit-run clean api web build
...
help:
	...
	@echo "  api               - Start FastAPI backend (trae-api)"
	@echo "  web               - Start Vite dev server (frontend)"
	@echo "  build             - Build frontend (served by API)"
...
web:
	cd frontend && (command -v npm >/dev/null || echo "npm not found, please install Node.js") && npm install && npm run dev
